...
class MobileNetV2(nn.Module):
    def __init__(self, num_classes=1000, width_mult=1.0,output_last_channel=1280):
        ...
        for layer_index,(t, c, n, s) in enumerate(inverted_residual_setting):
            ...
            if layer_index==1:
                ...
                for i in range(n):
                    stride = [s,1] if i == 0 else 1
                    features.append(block(input_channel, output_channel, stride, expand_ratio=t))
                    input_channel = output_channel

        # building last several layers
       # features.append(ConvBNReLU(input_channel, self.last_channel, kernel_size=1))
        features.append(ConvBNReLU(input_channel,output_last_channel, kernel_size=1))
        #features.append(nn.Conv2d(last_channel,output_last_channel,kernel_size=1))
...
def mobilenet_v2(config,pretrained=False, progress=True, **kwargs):
    """
    ...
    """
    model = MobileNetV2(output_last_channel=int(config.MODEL.LSTM_NUM_HIDDEN),**kwargs)
...
class MobileNetV3_Large(nn.Module):
    def __init__(self, config):
        ...
        self.hs2 = hswish()
        self.conv3=nn.Conv2d(960,int(config.MODEL.LSTM_NUM_HIDDEN),kernel_size=1)
